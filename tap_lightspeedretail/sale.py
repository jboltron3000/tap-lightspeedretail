...
class Sale(Stream):
    ...
    def create_relation(self, start_date):
        relation = "&archived=true&load_relations=%5B%22SaleLines%22%5D&timeStamp=%3E," +start_date
        return relation
    ...
    def paginate(self, offset, count, ext_time, path, stream_id):
        ...
        while (int(count) > int(offset) and (int(count) - int(offset)) >= -100):    
            ...
            for key in list(data):
                #pdb.set_trace()
                if 'SaleLines' not in key:
                    pass
                elif int(count) == 1: 
                    dict = data['SaleLines']['SaleLine']
